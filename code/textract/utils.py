...
def get_collection_status(aoss_client,name,region_name):
    while True:
        ...
        # print(f'collection_details response :{collection_details}')
...
def create_encryption_policy(aoss_client, encryption_policy_name, vector_store_name, type):

    try:
        # response = aoss_client.get_security_policy(name=encryption_policy_name , type=type)
    
        # print("Security Policy does not exist. Creating Security Policy")
...
def create_network_policy(aoss_client, network_policy_name, vector_store_name, type):
    try:
        # response = aoss_client.get_security_policy(name=network_policy_name , type=type)
    # except aoss_client.exceptions.ResourceNotFoundException:
...
def extract_text_using_textract(textract,bucket,key):
...
def store_data_from_documents(docs, embeddings, opensearch_url, index_name, awsauth):
...
def get_opensearch_client(embeddings, opensearch_url,index_name, awsauth):
...
def store_data_from_texts(docs, embeddings, opensearch_url, index_name, awsauth):
    print(f"Indexing {len(docs)} documents into {index_name}...")
